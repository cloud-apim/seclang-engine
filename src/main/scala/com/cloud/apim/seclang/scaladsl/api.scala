...
object SecLang {
  ...
  def engine(program: CompiledProgram, files: Map[String, String] = Map.empty, env: Map[String, String] = Map.empty, config: SecRulesEngineConfig = SecRulesEngineConfig.default): SecRulesEngine = new SecRulesEngine(program)
}